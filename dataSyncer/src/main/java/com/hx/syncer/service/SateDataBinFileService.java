...
public class SateDataBinFileService {
    ...
    public void readAndSaveFileBin(String tableName, String sfId, Path satePath) {
        ...
        try (InputStream in = new FileInputStream(satePath.toFile())) {
            ...
            log.error("error while reading binFile,binFilePath:" + satePath + ",tableName:" + tableName, e1);
        }
        try {
            Files.delete(satePath);
            log.info("delete binFile,filePath:" + satePath);
        } catch (Exception e) {
            log.error("error for delete binFile,binFilePath:" + satePath, e);
        }
    }
