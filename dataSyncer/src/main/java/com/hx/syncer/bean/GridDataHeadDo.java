 ...
 */
@Entity
@Table(name = "public.grid_data_head")
public class GridDataHeadDo {
    ...
    @Column(name = "retrieve_start_time",nullable=false)
    private Timestamp retrieve_start_time;//: "2017-07-20 09:16:47",  -----任务启动时间
    @Column(name = "retrieve_end_time",nullable=false)
    private Timestamp retrieve_end_time;//": "2017-07-20 09:18:01",   -----任务结束时间
    @Column(name = "retrieve_process_time",nullable=false)
    ...
    @Column(name = "gridfile_num_max",nullable=false)
    private long gridfileNumMax;
    @Column(name = "gridfile_state",nullable=false)
    private String gridfile_state;
    @Column(name = "longitude_latitude_scope",nullable=false)
    private String longitude_latitude_scope;//": "73.33,135.05:3.51,53.33",   ----资料的经纬度范围
    @Column(name = "data_num",nullable=false)
    private String data_num;//": "364526",   -----一次任务得到的资料站台数（预处理前的资料行数,即一行一个站的记录）
    @Column(name = "sys_conn_state",nullable=false)
    private String sys_conn_state;//": "0",  ------cimiss接口连接状态 0表示成功连接服务器
    @Column(name = "return_abnormal_info",nullable=false)
    ...
    @Column(name = "fcstLevel",nullable=false)
